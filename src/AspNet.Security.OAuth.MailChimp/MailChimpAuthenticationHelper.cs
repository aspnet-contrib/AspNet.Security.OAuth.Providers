...
namespace AspNet.Security.OAuth.MailChimp {
    ...
    public static class MailChimpAuthenticationHelper {
        ...
        /// </summary>
        public static string GetEmail([NotNull] JObject payload) => payload.Value<JObject>("login")?.Value<string>("email");

        /// <summary>
        /// Gets the login id corresponding to the authenticated user.
        /// </summary>
        public static string GetLoginId([NotNull] JObject payload) => payload.Value<JObject>("login")?.Value<string>("login_id");
        
        /// <summary>
        /// Gets the name corresponding to the authenticated user.
        /// </summary>
        public static string GetName([NotNull] JObject payload) => payload.GetValue("login").Value<string>("login_name");

        /// <summary>
        /// Gets the login email address corresponding to the authenticated user.
        /// </summary>
        public static string GetLoginEmail([NotNull] JObject payload) => payload.Value<JObject>("login")?.Value<string>("login_email");
