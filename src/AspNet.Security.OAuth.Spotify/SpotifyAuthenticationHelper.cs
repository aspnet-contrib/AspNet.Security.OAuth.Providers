...
namespace AspNet.Security.OAuth.Spotify {
    ...
    public static class SpotifyAuthenticationHelper {
        ...
        /// </summary>
        public static string GetLink([NotNull] JObject user) => user.Value<JObject>("external_urls")?.Value<string>("spotify");

        /// <summary>
        /// Gets the profile picture URL corresponding to the authenticated user.
        /// </summary>
        public static string GetProfilePictureUrl([NotNull] JObject user) => user.Value<JArray>("images")?.First?.Value<string>("url");
    }
}
